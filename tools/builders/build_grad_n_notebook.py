# -*- coding: utf-8 -*-
"""build_grad_n_notebook.py - write `notebooks/p4_networks/15_real_network_import.ipynb`,
...
OUT = os.path.join(ROOT, "notebooks", "p4_networks", "15_real_network_import.ipynb")
...
L("# Read from the VENDORED copy, not fetched. data/vendor/case_ACTIVSg2000.m.md"),
L("# carries its provenance, its sha256 and the licence position -- which is not"),
L("# simple: MATPOWER ships this file but its BSD licence explicitly does NOT"),
L("# cover case files, so the grant that matters is Texas A&M's own."),
L("CASE_PATH = Path('../../data/vendor') / f'{CASE}.m'"),
L("if CASE_PATH.exists():"),
L("    text = CASE_PATH.read_text(encoding='utf-8')"),
L("    print(f'read {CASE_PATH} (vendored, no network)')"),
L("else:   # a stranger running a single notebook without the repository"),
L("    url = ('https://raw.githubusercontent.com/MATPOWER/matpower/master/'"),
L("           f'data/{CASE}.m')"),
L("    text = requests.get(url, timeout=120).text"),
L("    print(f'fetched {url}')"),
