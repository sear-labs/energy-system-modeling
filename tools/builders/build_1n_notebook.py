# -*- coding: utf-8 -*-
"""build_1n_notebook.py - write `notebooks/p2_demand/06_end_use_disaggregation.ipynb`,
...
OUT = os.path.join(ROOT, "notebooks", "p2_demand", "06_end_use_disaggregation.ipynb")
...
A(code(
L("YEAR = 2030"),
L("# PINNED, per Part 1 rule 3. This URL used to read `master`, which is not"),
L("# a version: on 2026-09-13 master, v0.15.0 and v0.13.2 of costs_2030.csv"),
L("# were three different files (283,889 / 281,355 / 268,002 bytes). A"),
L("# notebook reading master gives whatever upstream happened to have that"),
L("# morning, so its answers are not reproducible even to the day."),
L("#"),
L("# The tables are GPL-3.0 -- a copyleft licence, read here rather than"),
L("# vendored. See data/vendor/README.md."),
L("TECH_DATA_VERSION = 'v0.15.0'"),
L("url = (f'https://raw.githubusercontent.com/PyPSA/technology-data/'"),
L("       f'{TECH_DATA_VERSION}/outputs/costs_{YEAR}.csv')"),
...
A(code(
L("RESOLUTION = 3  # hours"),
L("PROFILE_YEAR = 2021"),
L(""),
L("# Texas, not Germany. This used to read an hourly series for GERMANY 2015"),
L("# from a personal TU Berlin share link with no stated licence. Both halves"),
L("# of that were wrong for this course: the data was the wrong continent, and"),
L("# an unlicensed personal share link is not a source you can build teaching"),
L("# material on."),
L("#"),
L("# It now reads TX-123BT (Lu and Li, DOI 10.6084/m9.figshare.22144616.v6,"),
L("# CC BY 4.0), aggregated to one annual table by"),
L("# tools/build_tx123bt_profiles.py. See the sidecar beside the CSV for what"),
L("# the columns mean -- in particular that the p.u. series are normalised"),
L("# realised output rather than availability."),
L("PROFILES = (Path('../../data/vendor')"),
L("            / f'tx123bt_profiles_{PROFILE_YEAR}.csv')"),
L(""),
L("_raw = pd.read_csv(PROFILES, float_precision='round_trip')"),
L("_raw.index = pd.date_range(f'{PROFILE_YEAR}-01-01', periods=len(_raw),"),
L("                           freq='1h')"),
L("ts = _raw.rename(columns={'solar_pu': 'pv_pu'})[['load_mw', 'wind_pu',"),
L("                                                 'pv_pu']][::RESOLUTION]"),
L(""),
L("# 8,760 hourly rows in, one row every RESOLUTION hours out. Asserted"),
L("# because a silently short year would rescale every annual total below."),
L("assert len(_raw) == 8760, f'expected a full year, got {len(_raw)} rows'"),
L("assert ts.wind_pu.max() <= 1.0 and ts.pv_pu.max() <= 1.0"),
L(""),
L("print(f'{len(ts):,} snapshots at {RESOLUTION}h, '"),
L("      f'{ts.index[0]:%Y-%m-%d} to {ts.index[-1]:%Y-%m-%d}')"),
L("print(f'peak load {ts.load_mw.max():,.0f} MW   '"),
L("      f'mean {ts.load_mw.mean():,.0f} MW')"),
L("ts.head(3)"),
))
